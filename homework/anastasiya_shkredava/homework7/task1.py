...
def text_check(text, alphabet):
    ...
    for letter in text:
        if letter.isalpha() and alphabet.count(letter.lower()) == 0:
            return print(
              f'*Ваш текст содержит буквы не английского алфавита, которые не будут зашифрованы.' if user_language == 1
              else f'*Ваш текст содержит буквы не русского алфавита, которые не будут зашифрованы.'
            )
